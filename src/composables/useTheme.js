...
export function useTheme() {
    ...
    onUnmounted(() => {
        if (mql) {
            if (mql.removeEventListener) mql.removeEventListener('change', handlePrefChange)
            else if (mql.removeListener) mql.removeListener(handlePrefChange)
        }
    })

    const iconClass = computed(() => (isDark.value ? 'bi bi-moon-fill' : 'bi bi-sun-fill'))
    const buttonTitle = computed(() => (isDark.value ? 'Ativar modo claro' : 'Ativar modo escuro'))
