    ...
    <div class="container mt-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex gap-2">
                <input v-model="searchQuery" @keyup.enter="fetchAll" type="text" class="form-control"
                    placeholder="Pesquisar..." style="max-width: 250px" />
                <button class="btn btn-outline-primary" @click="fetchAll">Buscar</button>
                <button class="btn btn-outline-success" @click="fetchAll"><i class="bi bi-repeat"></i></button>
            </div>
        </div>
        ...
        <section>
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <div>
                    <button class="btn btn-sm btn-outline-secondary me-2" :disabled="!unreadsMeta.has_prev"
                        @click="changePage('unreads', unreadsMeta.page - 1)">
                        ← Anterior
                    </button>
                    <button class="btn btn-sm btn-outline-secondary" :disabled="!unreadsMeta.has_next"
                        @click="changePage('unreads', unreadsMeta.page + 1)">
                        Próxima →
                    </button>
                </div>
                ...
                <div v-for="notice in unreads" :key="notice.id" class="col-md-4 mb-3">
                    <div class="card shadow-sm h-100">
                        <div class="card-body d-flex flex-column">
                            ...
                            <button class="btn btn-sm btn-success w-100" :disabled="loadingIds.has(notice.id)"
        ...
        <section>
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <div>
                    <button class="btn btn-sm btn-outline-secondary me-2" :disabled="!readsMeta.has_prev"
                        @click="changePage('reads', readsMeta.page - 1)">
                        ← Anterior
                    </button>
                    <button class="btn btn-sm btn-outline-secondary" :disabled="!readsMeta.has_next"
                        @click="changePage('reads', readsMeta.page + 1)">
                        Próxima →
                    </button>
                </div>
                ...
                <table class="table table-striped table-hover align-middle">
                    ...
                    <tbody>
                        <tr v-for="notice in reads" :key="notice.id">
                            ...
                            <td class="text-truncate" style="max-width: 250px">{{ notice.content }}</td>
                            <td>{{ formatDate(notice.created_at) }}</td>
                            <td>
                                <router-link class="btn btn-outline-info btn-sm"
                        ...
                        <tr v-if="!reads.length">
                            <td colspan="5" class="text-center text-muted">Nenhum aviso lido ainda.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
const fetchUnreads = async (page = unreadsMeta.value.page) => {
    try {
        const res = await NoticesService.listUnreads((page - 1) * 10, 10, searchQuery.value)
...
const fetchReads = async (page = readsMeta.value.page) => {
    try {
        const res = await NoticesService.listReads((page - 1) * 10, 10, searchQuery.value)
