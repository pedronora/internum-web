    ...
    <div class="container mt-4">
        ...
        <div
            ...
            <div class="d-flex gap-2 w-md-auto">
                <input class="form-control" v-model="q" @keyup.enter="reload" placeholder="Pesquisar..." />
        ...
        <div v-else>
            <div v-if="!error" class="table-responsive shadow-sm rounded mb-4">
                <table class="table table-striped table-hover align-middle mb-0" v-if="notices.length">
                    ...
                    <tbody>
                        <tr v-for="n in notices" :key="n.id">
                            <td>{{ n.id }}</td>
                            <td>{{ n.title }}</td>
                            <td class="text-truncate" style="max-width: 250px">{{ n.content }}</td>
                            <td>
                                <span class="badge bg-info">{{ n.reads_count }}</span>
                            </td>
                            <td>{{ formatDate(n.created_at) }}</td>
                            <td class="text-nowrap">
                                <router-link :to="{ name: 'NoticesDetail', params: { id: n.id } }"
                                    class="btn btn-sm btn-outline-primary me-1">
                                    Ver
                                </router-link>
                                <button @click="deleteNotice(n.id)" class="btn btn-sm btn-outline-danger">
                                    Excluir
                                </button>
                            </td>
                        </tr>
...
async function deleteNotice(id) {
    const ok = await confirmToast('Deseja excluir este aviso?', { title: 'Excluir aviso' });
    if (!ok) return;
    ...
    try {
        ...
        console.error(err);
        errorToast(err?.response?.data?.detail || err?.message || 'Erro ao excluir aviso');
    }
}
